...
class FlowChart {
  ...
  mermaidGraph() {
    ...
    if (elements.length == 0) {
      throw new Error("invalid expression used.");
    }
...
ifClauseBtn.onclick = function(e) {
  ...
  try {
    ...
    newValue = flowChart.mermaidGraph();
...
function parseAsBinaryTree(formula) {
  ...
  if (
    formula.length < 3 ||
    formula.substring(0, 3).toLowerCase() != "if(" ||
...
function parseExpression(expression, delimiter) {
  ...
  while (currPos < expression.length) {
    ...
    if (currCh == ")") {
      ...
      if (popped === undefined) {
        throw new Error(
          `invalid brackets expression: ${expression.substring(
            startExprPos,
            currPos
          )}`
        );
  ...
  if (state != StateDone || brackets.length > 0) {
  ...
  return new IfExpression(
    exprClause,
    parseAsBinaryTree(exprYes),
    parseAsBinaryTree(exprNo)
  );
}
